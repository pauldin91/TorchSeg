...
C.log_dir = osp.join(os.getcwd(),'log')

C.log_dir_link = osp.join(C.abs_dir, 'log')
C.snapshot_dir = osp.abspath(osp.join(C.log_dir,C.channels))
...
C.dataset_path = "~/FlameT"
C.img_root_folder = C.dataset_path
C.gt_root_folder = C.dataset_path
C.train_source = osp.join(C.dataset_path,"lists",C.dataset, "train.txt")
C.eval_source = osp.join(C.dataset_path,"lists" ,C.dataset, "validation.txt")
C.test_source = osp.join(C.dataset_path, "lists",C.dataset,"test.txt")
C.is_test = False
...
#C.pretrained_model = "/home/pdinopoulos/TorchSeg-master/resnet18_v1.pth"
